...
plot_vsref<-function(d,ref,
  ...
  if(is.null(limits)){
    ...
    ds<-maxs-mins
    xlimits<-ylimits<-c(mins-0*ds*3/8,maxs+0*ds*3/8)
  }
  else{
...
plotsummary<-function(Td_noise,Td=NULL,Td_old=NULL,
  ...
  #fall TRUE allows all sequences to be plotted 
...
a_b_bagplot<-function(community.f=NULL,Td,
  ...
  if(is.null(community.f)){
    if(!is.null(bestintersectr)){
      bestintersectname<-rownames(Td$ET.x)[bestintersectr]
    } else { cat("\na_b_bagplot: bestintersectr not set\n"); bestintersectname<-NA}
  }
